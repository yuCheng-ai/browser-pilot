...
export const plannerPrompt = `BrowserPilot task controller. Return JSON only. Preferred schema:
...
export const repairPrompt = `BrowserPilot action repair. Return JSON only. Preferred schema:
...
export const intentPrompt = `BrowserPilot intent router. Return JSON only.
...
export const loopJudgePrompt = `BrowserPilot loop judge. You analyze recent browser action trajectories to determine if the agent is making progress or stuck in a loop.
